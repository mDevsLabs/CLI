...
Remote Control Server 是 mAI CLI 的远程控制后端，允许你通过浏览器 Web UI 远程监控和操作 mAI CLI 会话。

## 功能

- **会话管理** — 创建、监控、归档 mAI CLI 会话
- **实时消息流** — WebSocket / SSE 双向传输，实时查看对话和工具调用
- **权限审批** — 在 Web UI 中审批 mAI CLI 的工具权限请求
...
docker run -d \
  ...
  -v rcs-data:/app/data \
  ghcr.io/mai/remote-control-server:latest
```
