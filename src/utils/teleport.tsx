...
export async function teleportResumeCodeSession(
  ...
  try {
    ...
    if (!accessToken) {
      ...
      throw new Error(
        'mAI CLI web sessions require authentication with a Claude.ai account. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.',
      );
    }
...
export async function teleportFromSessionsAPI(
  ...
  try {
    ...
    if (axios.isAxiosError(error) && error.response?.status === 404) {
      ...
      throw new TeleportOperationError(
        `${sessionId} not found.`,
        `${sessionId} not found.\n${chalk.dim('Run /status in mAI CLI to check your account.')}`,
      );
    }
  ...
  try {
    ...
    if (!gitSource && bundleSeedGateOn) {
      ...
      if (!bundle.success) {
        ...
        const setup = repoInfo ? '. Please setup GitHub on https://mai-devs.vercel.app' : '';
        let msg: string;
