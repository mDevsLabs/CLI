...
/**
 * Get mAI CLI MCP configurations (excludes claude.ai servers from the
 ...
 * @returns mAI CLI server configurations with appropriate scopes
 */
  ...
  for (const [name, config] of Object.entries(schemaResult.data.mcpServers)) {
    ...
    if (
      ...
      errors.push({
        ...
        suggestion: `Change command to "cmd" with args ["/c", "npx", ...]. See: https://mai-devs.vercel.app`,
        mcpErrorMetadata: {
          scope,
          serverName: name,
          severity: 'warning',
