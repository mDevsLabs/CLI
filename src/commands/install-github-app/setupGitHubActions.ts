...
async function createWorkflowFile(
  ...
  if (createFileResult.code !== 0) {
    if (
      ...
      throw new Error(
        `Failed to create workflow file ${workflowPath}: A mAI workflow file already exists in this repository. Please remove it first or update it manually.`,
      )
    }
    ...
    const helpText =
      ...
      '· For manual setup → Visit: https://mai-devs.vercel.app'
    ...
    if (!skipWorkflow) {
      ...
      if (selectedWorkflows.includes('claude')) {
        workflows.push({
          ...
          message: 'mAI PR Assistant workflow',
        })
      }
      ...
      if (selectedWorkflows.includes('claude-review')) {
        workflows.push({
          ...
          message: 'mAI CLI Review workflow',
        })
      }
