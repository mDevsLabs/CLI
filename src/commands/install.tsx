...
function Install({ onDone, force, target }: InstallProps): React.ReactNode {
    ...
    async function run() {
      try {
        ...
        if (result.lockFailed) {
          throw new Error(
            'Could not install - another process is currently installing mAI CLI. Please try again in a moment.',
          );
        }
  ...
  useEffect(() => {
    if (state.type === 'success') {
      ...
      setTimeout(onDone, 2000, 'mAI CLI installation completed successfully', {
        display: 'system' as const,
      });
    } else if (state.type === 'error') {
      // Give error message time to render before exiting
      setTimeout(onDone, 3000, 'mAI CLI installation failed', {
        display: 'system' as const,
      });
    }
  }, [state, onDone]);
  ...
  return (
    <Box flexDirection="column" marginTop={1}>
      ...
      {state.type === 'installing' && <Text color="claude">Installing mAI CLI native build {state.version}...</Text>}
        ...
        <Box flexDirection="column" gap={1}>
          <Box>
            ...
            <Text color="success" bold>
              mAI CLI successfully installed!
            </Text>
          </Box>
...
export const install = {
  ...
  name: 'install',
  description: 'Install mAI CLI native build',
