    ...
    return (
      <PermissionDialog color="planMode" title="Exit plan mode?" workerBadge={workerBadge}>
        <Box flexDirection="column" paddingX={1} marginTop={1}>
          <Text>mAI wants to exit plan mode</Text>
          <Box marginTop={1}>
            <Select
              options={[
      ...
      <PermissionDialog color="planMode" title="Ready to code?" innerPaddingX={0} workerBadge={workerBadge}>
        <Box flexDirection="column" marginTop={1}>
          ...
          <Box flexDirection="column" paddingX={1}>
            ...
            {!useStickyFooter && (
              <>
                <Text dimColor>mAI has written up a plan and is ready to execute. Would you like to proceed?</Text>
                <Box marginTop={1}>
                  <Select
                    options={options}
...
export function buildPlanApprovalOptions({
  ...
  if (showUltraplan) {
    options.push({
      label: 'No, refine with Ultraplan on mAI CLI on the web',
      value: 'ultraplan',
    });
  }
  ...
  options.push({
    ...
    value: 'no',
    placeholder: 'Tell mAI what to change',
