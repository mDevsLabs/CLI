...
export function DesktopUpsellStartup({ onDone }: Props): React.ReactNode {
  ...
  const options = [
    { label: 'Open in mAI CLI Desktop', value: 'try' as const },
    { label: 'Not now', value: 'not-now' as const },
    { label: "Don't ask again", value: 'never' as const },
  ];

  return (
    <PermissionDialog title="Try mAI CLI Desktop">
      <Box flexDirection="column" paddingX={2} paddingY={1}>
        <Box marginBottom={1}>
          <Text>Same mAI CLI with visual diffs, live app preview, parallel sessions, and more.</Text>
        </Box>
